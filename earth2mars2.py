...
ctx_to_send_back = None
is_running = True

def received_instruction(instruction, ctx):
    if not is_on():
        return
    with lock:
        instruction_to_execute = instruction
        ctx_to_send_back = ctx

def is_on():
    with lock:
        return is_running

def ignore_instruction():
    with lock:
        is_running = False
...
def get_ctx():
    with lock:
        return ctx_to_send_back

def get_instruction():
    with lock:
        retval = instruction_to_execute
        instruction_to_execute = None
...
async def interpret_ctx_emoji(ctx):
    ...
    if instructions:
        received_instruction(instructions, ctx)
        send(instructions)
        feedback = receive()
        print(feedback)
